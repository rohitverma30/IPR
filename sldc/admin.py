...
from .models import sldcSubmission, sldc, banner_img

User = get_user_model()

@admin.register(sldcSubmission)
class sldcSubmissionAdmin(admin.ModelAdmin):
    list_display = ["name", "surname", "email", "phone", "title", "submitted_at"]
    search_fields = ["name", "surname", "email", "title"]
    list_filter = ["submitted_at"]

@admin.register(sldc)
class sldcAdmin(admin.ModelAdmin):
